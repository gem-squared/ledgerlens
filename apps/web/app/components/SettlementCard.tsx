...
export function SettlementCard({ settlement }: { settlement: SimulatedSettlement }) {
  ...
  return (
    <div className={`glass-panel ${settled ? 'border-emerald-500/30 bg-emerald-500/5' : ''}`}>
...
function Cell({ k, v, mono }: { k: string; v: string; mono?: boolean }) {
  return (
    <div className="flex items-baseline gap-2 min-w-0">
      ...
      <span className={`truncate text-xs ${mono ? 'font-mono text-zinc-300' : 'text-zinc-200'}`}>{v}</span>
    </div>
  );
}
